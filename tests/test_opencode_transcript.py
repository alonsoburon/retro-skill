...
def _database(path: str) -> None:
    ...
    for message_id, session_id, created, data in rows:
        conn.execute(
            "INSERT INTO message VALUES (?,?,?,?)", (message_id, session_id, created, json.dumps(data))
        )
    parts = [
        ...
        (
            ...
            "s1",
            {
                ...
                "tool": "bash",
                "id": "call-1",
                "state": {"status": "completed", "input": {"command": "ls"}, "output": "boom"},
            },
        ),
    ]
...
class OpencodeTranscriptTest(unittest.TestCase):
    ...
    def test_a_tool_part_becomes_a_tool_use_and_a_matching_tool_result(self) -> None:
        ...
        kinds = [block["type"] for line in lines for block in line["message"]["content"]]
        self.assertIn("tool_use", kinds)
        self.assertIn("tool_result", kinds)

        uses = [b for line in lines for b in line["message"]["content"] if b["type"] == "tool_use"]
        results = [
            b for line in lines for b in line["message"]["content"] if b["type"] == "tool_result"
        ]
    ...
    def test_the_session_is_found_by_content_and_an_unknown_token_is_refused(self) -> None:
        conn = adapter._connect(self.db)
    ...
    def test_the_detector_accepts_the_rendered_transcript(self) -> None:
        ...
        conn = adapter._connect(self.db)
        out = tempfile.NamedTemporaryFile(suffix=".jsonl", delete=False, mode="w", encoding="utf-8")
        out.write("\n".join(adapter.render(conn, "s1")) + "\n")
        out.close()

        events = detector.load_jsonl(Path(out.name))
